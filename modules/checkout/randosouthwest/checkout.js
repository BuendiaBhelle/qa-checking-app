...
async function checkout(domain, username, password, module_name, launch, range_product_name, timestamp, wp_creds_username, wp_creds_password, product_name, range_coupons, range_thankyou_page, product_link, co_site) {
    ...
    try {
        ...
        await driver.sleep(3000);
        await driver.executeScript("return document.getElementsByClassName('added_to_cart wc-forward')[0].click()");
       
        // track product name
        try {
            let product_item_name = await driver.executeScript("return document.getElementsByClassName('product-name')[1].innerText");
    ...
    try {
        ...
        let subscribe = await driver.executeScript("return document.getElementById('mailchimp_woocommerce_newsletter').checked");
        if (subscribe === true) {
            await driver.findElement(By.id("mailchimp_woocommerce_newsletter")).click();
        }
