...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let fname = await driver.executeScript("return document.getElementsByName('first-name')[0]");
        fname.sendKeys("Primeview");
        let lname = await driver.executeScript("return document.getElementsByName('last-name')[0]");
        lname.sendKeys("Test");
        let email = await driver.executeScript("return document.getElementsByName('contact-email')[0]");
        email.sendKeys("qa@primeview.com");
        let phone = await driver.executeScript("return document.getElementsByName('contact-phone')[0]");
        phone.sendKeys("4806480839");
        let message = await driver.executeScript("return document.getElementsByName('questions-comments')[0]");
        message.sendKeys("Please take note that this is a test submit form for Request a quote. Please disregard if received. Thank you.");
