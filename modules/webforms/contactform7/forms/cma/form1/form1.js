...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let fullname = await driver.executeScript("return document.getElementsByName('cma-name')[0]");
        fullname.sendKeys("Primeview Test");
        await driver.sleep(1000);

        let email = await driver.executeScript("return document.getElementsByName('cma-email')[0]");
        email.sendKeys("qa@primeview.com");
        await driver.sleep(1000);

        let phone = await driver.executeScript("return document.getElementsByName('cma-phone')[0]");
        phone.sendKeys("4806480839");
        await driver.sleep(1000);

        let subject = await driver.executeScript("return document.getElementsByName('cma-subject')[0]");
        subject.sendKeys("Lead Test Submission");
        await driver.sleep(1000);

        let message = await driver.executeScript("return document.getElementsByName('cma-message')[0]");
        message.sendKeys("Please take note that this is a test submit form for CMA - Contact Form. Please disregard if received. Thank you.");
        await driver.sleep(1000);
