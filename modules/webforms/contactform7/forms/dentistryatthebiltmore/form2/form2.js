...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let name = await driver.executeScript("return document.getElementsByName('sb-name')[0]");
        name.sendKeys("Primeview Test");

        let contact = await driver.executeScript("return document.getElementsByName('sb-number')[0]");
        contact.sendKeys("4806480839");

        let email = await driver.executeScript("return document.getElementsByName('sb-email')[0]");
        email.sendKeys("qa@primeview.com");

        let message = await driver.executeScript("return document.getElementsByName('sb-message')[0]");
        message.sendKeys("Please take note that this is a test submit form for Get Started!. Please disregard if received. Thank you.");
