...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let phone = await driver.executeScript("return document.getElementsByName('your-contact-no')[0]");
        phone.sendKeys("4806480839");
        let message = await driver.executeScript("return document.getElementsByName('your-message')[0]");
        message.sendKeys("Please take note that this is a test submit form for Contact Us ( Form ). Please disregard if received. Thank you.");
