...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        date.sendKeys("05/10/2022" + Key.ENTER);
        ...
        let alp = await driver.executeScript("return document.getElementsByName('auth-liscence-plate')[0]");
        alp.sendKeys("Lead Test Submission");

        let av = await driver.executeScript("return document.getElementsByName('auth-vin')[0]");
        av.sendKeys("Lead Test Submission");
        ...
        let apd = await driver.executeScript("return document.getElementsByName('auth-prior-damage')[0]");
        apd.sendKeys("Lead Test Submission");

        let acr = await driver.executeScript("return document.getElementsByName('auth-customer-request')[0]");
        acr.sendKeys("Lead Test Submission");
