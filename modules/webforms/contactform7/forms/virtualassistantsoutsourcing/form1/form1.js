...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let name = await driver.executeScript("return document.getElementsByName('va-name')[0]");
        name.sendKeys("Primeview Test");

        let email = await driver.executeScript("return document.getElementsByName('va-email')[0]");
        email.sendKeys("qa@primeview.com");

        let subject = await driver.executeScript("return document.getElementsByName('va-subject')[0]");
        subject.sendKeys("Lead Test Submission");

        let message = await driver.executeScript("return document.getElementsByName('va-message')[0]");
        message.sendKeys("Please take note that this is a test submit form for Get in Touch with Us - Bottom. Please disregard if received. Thank you.");
