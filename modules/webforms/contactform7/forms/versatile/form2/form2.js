...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let phone = await driver.executeScript("return document.getElementsByName('design-phone')[0]");
        phone.sendKeys("4806480839");
        let zip = await driver.executeScript("return document.getElementsByName('design-zipcode')[0]");
        zip.sendKeys("4806480839");
        let message = await driver.executeScript("return document.getElementsByName('design-additional-info')[0]");
        message.sendKeys("Please take note that this is a test submit form for Custom Tiles Form. Please disregard if received. Thank you.");
        let area = await driver.executeScript("return document.getElementsByName('design-est-area')[0]");
        area.sendKeys("24");
        let date = await driver.executeScript("return document.getElementsByName('design-est-time')[0]");
        date.sendKeys("04/25/2022");
