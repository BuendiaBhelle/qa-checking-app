...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let name = await driver.executeScript("return document.getElementsByName('nw-name')[0]");
        name.sendKeys("Primeview Test");
        let email = await driver.executeScript("return document.getElementsByName('nw-email')[0]");
        email.sendKeys("qa@primeview.com");
        await driver.executeScript("return document.getElementsByClassName('btn-primary')[4].click()");
