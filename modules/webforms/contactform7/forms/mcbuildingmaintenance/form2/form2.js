...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let phone = await driver.executeScript("return document.getElementsByName('phone-number')[0]");
        phone.sendKeys("4806480839");

        await driver.executeScript("return document.getElementsByName('services')[0].click()");
        let services = await driver.executeScript("return document.getElementsByName('services')[0]");
        services.sendKeys(Key.ARROW_DOWN + Key.ENTER);

        let address = await driver.executeScript("return document.getElementsByName('your-address')[0]");
        ...
        message.sendKeys("Please take note that this is a test submit form for Sidebar Form. Please disregard if received. Thank you.");
