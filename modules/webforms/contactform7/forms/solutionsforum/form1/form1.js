...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let name = await driver.executeScript("return document.getElementsByName('full_name')[0]");
        name.sendKeys("Primeview Test");
        let title = await driver.executeScript("return document.getElementsByName('title')[0]");
        title.sendKeys("Lead Test Submission");
        let company = await driver.executeScript("return document.getElementsByName('company')[0]");
        company.sendKeys("Test");
        let address = await driver.executeScript("return document.getElementsByName('address')[0]");
        ...
        let comment = await driver.executeScript("return document.getElementsByName('comment')[0]");
        comment.sendKeys("Please take note that this is a test submit form for Contact form 1. Please disregard if received. Thank you.");
