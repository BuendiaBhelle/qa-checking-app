...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        await driver.executeScript("return document.getElementsByClassName('elementor-button-text')[3].click()");
        await driver.sleep(3000);
        let name = await driver.executeScript("return document.getElementsByName('your-name')[0]");
        name.sendKeys("Primeview Test");
        let phone_number = await driver.executeScript("return document.getElementsByName('your-phone')[0]");
        ...
        let services = await driver.executeScript("return document.getElementsByName('your-services')[0]");
        services.sendKeys("Guided Hunting - Quail Hunt");
        let date = await driver.executeScript("return document.getElementsByName('your-date')[0]");
        date.sendKeys("08/02/2023");
        let time = await driver.executeScript("return document.getElementsByName('your-time')[0]");
