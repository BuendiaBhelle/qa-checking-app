...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        await driver.sleep(3000);
        let name = await driver.executeScript("return document.getElementsByName('your-name')[3]");
        name.sendKeys("Primeview Test");
        let phone_number = await driver.executeScript("return document.getElementsByName('your-phone')[3]");
        phone_number.sendKeys("4806480839");
        let email = await driver.executeScript("return document.getElementsByName('your-email')[3]");
        email.sendKeys("qa@primeview.com");
        let services = await driver.executeScript("return document.getElementsByName('your-services')[3]");
        services.sendKeys("Overnight Stays - RV Camping");
        let date = await driver.executeScript("return document.getElementsByName('your-date')[3]");
        date.sendKeys("08/02/2023");
        let time = await driver.executeScript("return document.getElementsByName('your-time')[3]");
        time.sendKeys("10:00AM");
        let message = await driver.executeScript("return document.getElementsByName('your-message')[3]");
        message.sendKeys("This is a test. Please ignore.");
        await driver.executeScript("return document.getElementsByClassName('wpcf7-form-control has-spinner wpcf7-submit')[3].click()");
