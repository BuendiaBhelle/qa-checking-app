...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let first_name = await driver.executeScript("return document.getElementsByName('txtFName')[0]");
        first_name.sendKeys("Primeview");
        await driver.sleep(1000);
        let last_name = await driver.executeScript("return document.getElementsByName('txtLName')[0]");
        last_name.sendKeys("Test");
        await driver.sleep(1000);
        let contact_no = await driver.executeScript("return document.getElementsByName('txtPhone')[0]");
        contact_no.sendKeys("4806480839");
        await driver.sleep(1000);
        let email = await driver.executeScript("return document.getElementsByName('txtEmail')[0]");
        email.sendKeys("qa@primeview.com");
        await driver.sleep(1000);
        let message = await driver.executeScript("return document.getElementsByName('txtMessage')[0]");
        message.sendKeys("Please take note that this is a test submit form for Contact form @ Contact Us Page Form. Please disregard if received. Thank you.");
        await driver.sleep(1000);
