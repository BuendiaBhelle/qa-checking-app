...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let name = await driver.executeScript("return document.getElementsByName('cperson')[0]");
        name.sendKeys("Primeview Test");
        let email = await driver.executeScript("return document.getElementsByName('eaddress')[0]");
        email.sendKeys("qa@primeview.com");
        let message = await driver.executeScript("return document.getElementsByName('siteurl')[0]");
        message.sendKeys("Please take note that this is a test submit form for PDF Form. Please disregard if received. Thank you.");
        await driver.executeScript("return document.getElementsByName('checkbox-197[]')[1].click()");
