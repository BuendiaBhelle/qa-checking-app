...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        let name = await driver.executeScript("return document.getElementsByName('lname')[0]");
        name.sendKeys("Test");
        let email = await driver.executeScript("return document.getElementsByName('email')[0]");
        email.sendKeys("qa@primeview.com");
        let phone = await driver.executeScript("return document.getElementsByName('pnumber')[0]");
        phone.sendKeys("4806480839");
        let address = await driver.executeScript("return document.getElementsByName('address1')[0]");
        address.sendKeys("7620 E McKellips Rd");
        let city = await driver.executeScript("return document.getElementsByName('citytest')[0]");
        city.sendKeys("Scottsdale");
        let state = await driver.executeScript("return document.getElementsByName('spr')[0]");
        state.sendKeys("AZ");
        let zip = await driver.executeScript("return document.getElementsByName('zpc')[0]");
        zip.sendKeys("85257");
        let message = await driver.executeScript("return document.getElementsByName('addinfo')[0]");
        message.sendKeys("Please take note that this is a test submit form for Free Website Quote. Please disregard if received. Thank you.");
        await driver.executeScript("return document.getElementsByName('checkbox-197[]')[0].click()");
