...
async function webforms(domain, timestamp, forms, range_thankyou_page, module_name, launch, webforms) {
    ...
    try {
        ...
        fname.sendKeys("Primeview Test");
        let email = await driver.executeScript("return document.getElementsByName('your-email')[0]");
        email.sendKeys("qa@primeview.com");
        let message = await driver.executeScript("return document.getElementsByName('your-service')[0]");
        message.sendKeys("Please take note that this is a test submit form for Crankwheel Form. Please disregard if received. Thank you.");
        await driver.executeScript("return document.getElementsByClassName('wpcf7-form-control has-spinner wpcf7-submit crankwheel-com-showu-launch-button')[0].click()");
