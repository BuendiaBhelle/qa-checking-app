...
async function wordpressFinish(domain, checkbox, username, password, timestamp) {
    ...
    try {
        ...
        value = [ "", "info", "put original form recipients success", server.userId, timestamp, module_name, domain, "", "", "", launch, "", form_page_dev + "\n" + form, "", "" ];
        await sheet.addRow();
        await sheet.appendValues(value);
    } catch (error) {
        logger.logger.log({ level: 'error', message: error, tester: server.userId });
    ...
    value = [ "", "info", "test ends.", server.userId, timestamp, module_name, domain, "", "", "", launch, "", form_page_dev + "\n" + form, "", "" ];
    await sheet.addRow();
    await sheet.appendValues(value);
