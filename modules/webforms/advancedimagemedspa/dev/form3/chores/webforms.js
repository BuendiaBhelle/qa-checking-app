...
async function webforms(domain, checkbox, timestamp) {
    ...
    try {
        ...
        value = [ "", "info", "form fill in success.", server.userId, timestamp, module_name, domain, "", "", "", launch, "", form_page_dev + "\n" + form, "", "" ];
        await sheet.addRow();
    ...
    try {
        ...
        value = [ "", "info", "track thank you page success.", server.userId, timestamp, module_name, domain, "", "", "", launch, "", form_page_dev + "\n" + form, "", "" ];
        await sheet.addRow();
        await sheet.appendValues(value);
    } catch (error) {
        logger.logger.log({ level: 'error', message: error, tester: server.userId });
